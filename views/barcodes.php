...
<div class="col-md-12">
    <div class="row">
        <div class="col-sm-9 col-md-9 col-lg-10 col-main">
			<div style="float:left; height:100%; width:100%">
				...
				<div class="panel panel-default"  style="height:49%; background:#FFFFFF">
					<div class="panel-heading"><i class="fa fa-list" aria-hidden="true"></i> <strong>ELENCO PRODOTTI</strong></div>
						<div class="panel-body" style="height:86%; overflow-y: auto;">
							...
							<table id="tab_tipologie" class="table table-responsive table-hover table-striped">
								<thead>
									...
									<th width="5%">Pti</th>
									<th width="5%">Stock</th>
									<th width="10%">Liv. riordino</th>
									<th width="10%">Liv. critico</th>
									<th>&nbsp;</th>
                                    </tr>
								</thead>
								<tbody>
									<?php
										foreach($ris as $key=>$val) {
											...
											print("<tr class='clickable-row ".$alert_riga."' href='#dati_barcode' 
											    ...
											    print("<td style='text-align:right'>
															<div class=\"btn-group\">
																<button type=\"button\" class=\"btn btn-sm btn-default dropdown-toggle\" data-toggle=\"dropdown\" aria-expanded=\"false\">
																	<i class='fa fa-gear fa-fw'></i> Opzioni <span class=\"caret\"></span>
																</button>
																<ul class=\"dropdown-menu pull-right\" role=\"menu\">
																	<li><a href='' class='modifica' form-action='/tipologie/modifica/".$val['id_tipologia']."' 
																		id='".$val['id_tipologia']."' name=\"".$val['id_tipologia']."\">
        ...
        <div class="col-sm-3 col-md-3 col-lg-2 col-right">&nbsp;
			...
			<form id="cerca" method="POST" action="/barcodes/cerca">
                ...
                <div>
                  <table width="100%" border="0" cellpadding="5" cellspacing="5">
                    <tr>
				        <td><label class="checkbox-inline"><input type="checkbox" name="riordino" id="riordino" value="riordino">Riordino</label> </td>
				        <td><label class="checkbox-inline"><input type="checkbox" name="esaurimento" id="esaurimento" value="esaurimento">Esaurimento</label> </td>
                    </tr>
                    <tr>
                        <td colspan="2"><label class="checkbox-inline"><input type="checkbox" name="stock_esaurito" id="stock_esaurito" value="">Prodotti esauriti (barcodes)</label> </td>
                    </tr>
                    <tr>
                        <td colspan="2"><label class="checkbox-inline"><input type="checkbox" name="stock_negativo" id="stock_negativo" value="">Stock negativo (barcodes)</label> </td>
                    </tr>
                    <tr>
                        <td colspan="2"><label class="checkbox-inline"><input type="checkbox" name="agea" id="agea" value="">AGEA</label> </td>
                    </tr>
                  </table>
                </div>
            ...
            ?>
                <div>&nbsp;</div>
                <a class="btn btn-danger btn-sm btn-block" href="/barcodes/index"><i class="fa fa-times" aria-hidden="true"></i> Azzera risultati ricerca</a>
            <?php
                }
            ?>

			<div>&nbsp;</div>
			<a href="/barcodes/nuovo" class="btn btn-success btn-sm btn-block" role="button"><i class="fa fa-plus" aria-hidden="true"></i> Inserisci prodotto</a>

            <?php if($_SESSION['ruolo'] == "superlocale" || $_SESSION['ruolo'] == "super") { ?>
            <div>&nbsp;</div>
            <div>&nbsp;</div>
            <a href="/barcodes/esporta" class="btn btn-default btn-sm btn-block"><i class="fa fa-file-excel-o fa-lg" aria-hidden="true"></i> Esporta lista prodotti</a>
            <?php } ?>
...
$(document).ready(function(){
    $('.clickable-row').click(function(e){
        ...
        <?php if($cerca != 1) { ?>
			data: "id=" + tipologia,    
        <?php } else { ?>
            data: { "cerca_barcode": "<?php echo $cerca_barcode; ?>", "cerca_nome": "<?php echo $cerca_nome; ?>",
                    "id": tipologia, "agea": "<?php echo $agea; ?>", "stock_negativo": "<?php echo $stock_negativo; ?>",
                    "stock_esaurito": "<?php echo $stock_esaurito; ?>" },
        <?php } ?>
            cache: false,
            success: function(message) {
				...
				$('#nuova_qta').bind('keyup change input', function(e){
					...
					if(check.test(bla) && $('#motivazione').val() != '') {
                ...
                $('#motivazione').bind('keyup change input', function(e){
                    e.preventDefault(); 
                    motivazione = $('#motivazione').val(); // ma perchè, poi?
                    var check = new RegExp("^[A-Za-z 0-9,;:-_\\.àèéìùò']+$");

                    if(check.test(bla) && $('#nuova_qta').val() != '') {
                        $('#submit_qta').prop('disabled', false);
				...
				$('.offer_barcode').bind('click', function(e) {
                    e.preventDefault();
                    var action = $(this).attr('form-action');
                    $('#fake-form').attr('action', action);
                    $("#fake-form").submit();
				...
				$('.view_offer_barcode').bind('click', function(e) {
                    e.preventDefault();
                    var action = $(this).attr('form-action');
                    $('#fake-form').attr('action', action);
                    $("#fake-form").submit();
