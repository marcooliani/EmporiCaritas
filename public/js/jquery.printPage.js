...
(function( $ ){
  $.fn.printPage = function(options) {
    ...
    // this.one fa passare un solo click. In questo modo cerco di prevenire gli invii multipli
    // dovuti a doppi click involontari...
    //
    // this.one("click", function(){  loadPrintDocument(this, pluginOptions); return false;  });
    ...
    function loadPrintDocument(el, pluginOptions){
      $(".stampascontrino").prop('disabled', true);
    ...
    function addIframeToPage(el, pluginOptions){
        ...
        if(!$('#printPage')[0]){
          ...
          $('#printPage').on("load",function() {  
				printit();
                // Parte aggiunta per gli empori (in originale non c'è).
                // Togliere le tre righe seguenti per ripristinare lo script
                // come era in origine 
				var pathname = window.location.pathname; 
				if(pathname.split("/", 2).join("") == "cassa") {
					setTimeout(window.location.href = "/cassa/index", 2000);} 
			})
        }else{
